  ...
  if (submitted) {
    return (
      <div style={{ textAlign: 'center', padding: '2rem 1rem' }}>
        ...
        <h3 style={{ color: dark ? 'white' : '#1A1A1A', fontWeight: 600, fontSize: '1.15rem', marginTop: 14, marginBottom: 6 }}>
          Request Sent!
        </h3>
        <p style={{ color: dark ? 'rgba(255,255,255,0.5)' : '#999', fontSize: '0.9rem' }}>
  ...
  return (
    ...
    >
      ...
      <select
        name="entry.service"
        defaultValue=""
        style={{ ...inputStyle, appearance: 'none', cursor: 'pointer', color: dark ? 'rgba(255,255,255,0.5)' : '#999' }}
        onFocus={(e) => e.currentTarget.style.borderColor = focusBorder}
        onBlur={(e) => e.currentTarget.style.borderColor = defaultBorder}
        onChange={(e) => e.currentTarget.style.color = dark ? 'white' : '#1A1A1A'}
      >
        ...
        <option value="newbuild">New Build &amp; Renovation</option>
      ...
      <style>{`
        ...
        .quote-submit-btn:hover { background: #CC8C1A !important; transform: translateY(-1px); }
